...
function setPublicBaseUrl(url) {
  publicBaseUrl = String(url || "").trim() || window.location.origin;
  publicBaseUrlEl.textContent = publicBaseUrl;
}
...
socket.on("link:code", ({ code, expiresInSeconds }) => {
  ...
  setConnectInfo(
    `In Roblox eingeben: /caelus ${code} (gueltig ca. ${validMinutes} min) | Server: ${publicBaseUrl}`,
    "success"
  );
});
